...
// ── research_attendees ───────────────────────────────────────────────────────
server.tool("research_attendees", "Generate a background brief for meeting attendees based on their names and companies", {
...
// ── build_agenda ─────────────────────────────────────────────────────────────
server.tool("build_agenda", "Generate a structured meeting agenda from purpose and attendee context", {
...
// ── talking_points ────────────────────────────────────────────────────────────
server.tool("talking_points", "Generate key talking points and supporting data for each agenda item", {
...
// ── draft_followup ────────────────────────────────────────────────────────────
server.tool("draft_followup", "Draft a professional follow-up email from meeting notes, capturing decisions and next steps", {
...
// ── meeting_summary ───────────────────────────────────────────────────────────
server.tool("meeting_summary", "Extract structured action items, decisions, and key takeaways from meeting transcript or notes", {
    ...
    for (const pattern of actionPatterns) {
        ...
        if (matches)
            extractedActions.push(...matches.slice(0, 5));
    }
    ...
    for (const pattern of decisionPatterns) {
        const matches = text.match(pattern);
        if (matches)
            extractedDecisions.push(...matches.slice(0, 5));
    }
