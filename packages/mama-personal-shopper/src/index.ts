...
const server = new McpServer({ name: "mama_personal_shopper", version: "1.0.0" });

server.tool(
  "search_products",
  "Search for products matching a description",
  { input: z.string().describe("Input for search_products") },
  async ({ input }) => {
    const seed = `search_products:${input}`;
...
server.tool(
  ...
  { input: z.string().describe("Input for compare_prices") },
  async ({ input }) => {
    const seed = `compare_prices:${input}`;
    const score = rangeInt(60, 98, seed, 0);
    const items = Array.from({ length: rangeInt(3, 8, seed, 1) }, (_, i) => {
      const label = pick(["Analysis", "Recommendation", "Finding", "Insight", "Action Item", "Result"], seed, i + 10);
      const detail = pick([
...
server.tool(
  "find_deals",
  "Find current deals and coupons in a category",
  { input: z.string().describe("Input for find_deals") },
  async ({ input }) => {
    const seed = `find_deals:${input}`;
    const score = rangeInt(60, 98, seed, 0);
    const items = Array.from({ length: rangeInt(3, 8, seed, 1) }, (_, i) => {
      const label = pick(["Analysis", "Recommendation", "Finding", "Insight", "Action Item", "Result"], seed, i + 10);
      const detail = pick([
...
server.tool(
  "gift_finder",
  "Find gift ideas based on recipient profile and budget",
  { input: z.string().describe("Input for gift_finder") },
  async ({ input }) => {
    const seed = `gift_finder:${input}`;
    const score = rangeInt(60, 98, seed, 0);
    const items = Array.from({ length: rangeInt(3, 8, seed, 1) }, (_, i) => {
      const label = pick(["Analysis", "Recommendation", "Finding", "Insight", "Action Item", "Result"], seed, i + 10);
      const detail = pick([
...
server.tool(
  "style_match",
  "Find matching items for an outfit or style",
  { input: z.string().describe("Input for style_match") },
  async ({ input }) => {
    const seed = `style_match:${input}`;
    const score = rangeInt(60, 98, seed, 0);
    const items = Array.from({ length: rangeInt(3, 8, seed, 1) }, (_, i) => {
      const label = pick(["Analysis", "Recommendation", "Finding", "Insight", "Action Item", "Result"], seed, i + 10);
      const detail = pick([
...
server.tool(
  "track_price",
  "Set up a price alert for a product",
  { input: z.string().describe("Input for track_price") },
  async ({ input }) => {
    const seed = `track_price:${input}`;
    const score = rangeInt(60, 98, seed, 0);
    const items = Array.from({ length: rangeInt(3, 8, seed, 1) }, (_, i) => {
      const label = pick(["Analysis", "Recommendation", "Finding", "Insight", "Action Item", "Result"], seed, i + 10);
      const detail = pick([
