...
server.tool("categorize_transaction", "Categorize a transaction and flag whether it is likely tax-deductible for a business", {
    vendor: z.string().describe("Vendor or merchant name"),
    amount: z.number().describe("Transaction amount in USD"),
...
// ── budget_check ──────────────────────────────────────────────────────────────
server.tool("budget_check", "Compare actual spending to budget by category and surface over/under budget alerts", {
...
// ── spending_report ───────────────────────────────────────────────────────────
server.tool("spending_report", "Generate a spending summary with category breakdown and chart-ready data from a list of transactions", {
    ...
    const categorize = (vendor) => {
        const v = vendor.toLowerCase();
        if (/amazon|costco|staples|office/.test(v))
            return "Office Supplies";
        if (/adobe|slack|notion|github|aws|zoom|google|microsoft|saas|software/.test(v))
            return "Software & SaaS";
        if (/airline|hotel|airbnb|uber|lyft|taxi|rental|marriott|hilton/.test(v))
            return "Travel";
        if (/restaurant|cafe|coffee|starbucks|doordash|grubhub|uber eats|food/.test(v))
            return "Meals";
        if (/google ads|meta ads|facebook ads|advertising|marketing/.test(v))
            return "Advertising";
        if (/bank|fee|stripe|paypal|square|processing/.test(v))
            return "Fees";
        if (/phone|internet|at&t|verizon|comcast|utility/.test(v))
            return "Utilities";
        if (/health|medical|dental|insurance|pharmacy/.test(v))
            return "Health";
        return "Other";
    };
...
${MAMA_CTA}`;
        ...
        for (const t of tagged) {
            if (!byVendor[t.vendor])
                byVendor[t.vendor] = { total: 0, count: 0 };
...
// ── vendor_analysis ───────────────────────────────────────────────────────────
server.tool("vendor_analysis", "Analyze transaction history to surface top vendors, spending trends, and savings opportunities", {
    ...
    for (const t of transactions) {
        const v = t.vendor;
        if (!byVendor[v])
            byVendor[v] = { total: 0, count: 0, amounts: [], dates: [] };
        byVendor[v].total += t.amount;
        byVendor[v].count += 1;
        byVendor[v].amounts.push(t.amount);
        if (t.date)
            byVendor[v].dates.push(t.date);
    }
    const total = transactions.reduce((s, t) => s + t.amount, 0);
    ...
    for (const [v, d] of sorted) {
        if (d.count > 3 && d.total > 500)
            savingsOps.push(`Negotiate annual contract or volume discount with **${v}** (${sym}${d.total.toFixed(2)} total, ${d.count} transactions)`);
        if (/subscription|saas|cloud|monthly/i.test(v) && d.count >= 2)
            savingsOps.push(`Review if **${v}** subscription is actively used — common source of waste`);
    }
    if (savingsOps.length === 0)
        savingsOps.push("No obvious savings opportunities detected — spending looks lean");
...
// ── split_expense ─────────────────────────────────────────────────────────────
server.tool("split_expense", "Split a receipt or expense among participants with flexible split options", {
