...
server.tool(
  "write_job_post",
  "Generate a polished, compelling job listing from role details and requirements",
  {
...
server.tool(
  "screen_resume",
  "Score a resume against job requirements and return structured pros, cons, and hiring recommendation",
  {
  ...
  async ({ resume_text, job_requirements, nice_to_haves, role_title, seniority_level }) => {
    ...
    if (hasLeadership) pros.push("Shows leadership experience");
    if (hasMetrics) pros.push("Quantifies impact with metrics");
    if (hasDegree) pros.push("Has relevant degree");
    if (niceCount > 0) pros.push(`Meets ${niceCount}/${niceToHaveResults.length} nice-to-haves`);

    const seniorityExpMap: Record<string, [number, number]> = {
    ...
    if (yearsExp !== null) {
      if (yearsExp < minYrs) cons.push(`May be underqualified: ~${yearsExp} years vs. ${minYrs}+ expected for ${seniority_level}`);
      if (yearsExp > maxYrs + 3) pros.push(`Overqualified: ${yearsExp} years experience`);
    }
...
server.tool(
  "interview_questions",
  "Generate a structured set of behavioral and technical interview questions for a role",
  {
  ...
  async ({ role_title, seniority_level, focus_areas, interview_type }) => {
    ...
    const technicalByRole: Record<string, string[]> = {
      engineer: [
    ...
    const focusQs = (focus_areas ?? []).map((area) =>
      `[${area}] Describe a specific situation where your ${area} skills made a decisive difference.`
    );
...
server.tool(
  "comp_benchmark",
  "Get a market salary range for a role based on title, location, and seniority",
  {
  ...
  async ({ role_title, location, seniority_level, company_stage }) => {
    ...
    const baseBands: Record<string, Record<string, [number, number]>> = {
      engineer: {
...
server.tool(
  "offer_letter_draft",
  "Generate a professional offer letter from candidate, role, and compensation details",
  {
