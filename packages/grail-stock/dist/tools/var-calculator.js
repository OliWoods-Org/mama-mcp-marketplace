import { z } from "zod";
import { seeded, normTicker } from "../heuristics.js";
...
function portfolioCorrelation(tickers) {
    if (tickers.length === 1)
        return 1.0;
...
export function varCalculator(input) {
    ...
    const weightedVol = positions.reduce((s, p) => {
    ...
    const concentrationRisk = maxWeightPos.value / totalValue > 0.3
    ...
    const result = {
        ...
        interpretation: {
            ...
            stress_note: "VaR is a statistical estimate. Fat-tail events (2008, 2020 COVID crash) regularly exceed modelled VaR. Consider stress-testing against −30% scenarios.",
        },
