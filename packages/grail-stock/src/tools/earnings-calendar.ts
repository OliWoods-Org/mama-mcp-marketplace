import { z } from "zod";
import { seeded, seededInt, seededPick, normTicker, seededFutureDays, formatDate } from "../heuristics.js";
...
function earningsEntry(ticker: string, daysOffset: number) {
...
export function earningsCalendar(input: z.infer<typeof EarningsCalendarInput>): string {
  const { ticker } = input;
  ...
  const result = {
    ...
    high_impact_count: events.filter(
      (e) => e.options_implied_move_pct >= 8
    ).length,
    coverage_note:
      "Estimates sourced from analyst consensus; implied moves from near-term options pricing. Grail signals are probabilistic — not financial advice.",
