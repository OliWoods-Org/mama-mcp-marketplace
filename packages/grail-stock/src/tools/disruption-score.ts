import { z } from "zod";
import { seeded, seededInt, seededPick, normTicker } from "../heuristics.js";
...
const HISTORICAL_DISRUPTIONS: Record<string, { analog: string; outcome: string }[]> = {
  automotive: [
...
function inferSector(company: string): string {
  ...
  if (/ford|gm|toyota|honda|tesla|rivian|nio|stellantis|bmw|mercedes/.test(c)) return "automotive";
  if (/walmart|target|amazon|costco|kroger|dollar|sears|macy|jcpenney/.test(c)) return "retail";
  if (/apple|microsoft|google|meta|amazon|nvidia|intel|ibm|yahoo|oracle|salesforce/.test(c)) return "technology";
  if (/jpmorgan|bank|goldman|morgan|citi|wells|blackrock|visa|mastercard|paypal/.test(c)) return "financials";
  if (/exxon|chevron|bp|shell|conocophillips|halliburton|schlumberger|coal|oil/.test(c)) return "energy";
  if (/pfizer|johnson|merck|abbvie|eli lilly|unitedhealth|cvs|walgreen|hospital/.test(c)) return "healthcare";
  return "default";
}

export function disruptionScore(input: z.infer<typeof DisruptionScoreInput>): string {
