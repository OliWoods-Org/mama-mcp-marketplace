...
server.tool(
  "receipt_scanner",
  "Extract structured data from raw receipt text: vendor, date, line items, subtotal, tax, and total",
  {
  ...
  async ({ receipt_text, currency }) => {
    ...
    const amountLines: { description: string; amount: number }[] = [];
    ...
    for (const line of lines) {
      ...
      if (/subtotal|sub-total/.test(lower)) { subtotal = amount; continue; }
      if (/\btax\b|gst|vat|hst/.test(lower)) { tax = amount; continue; }
      if (/total|amount due|balance/.test(lower)) { total = amount; continue; }
      if (amount > 0) amountLines.push({ description: line.replace(/[\$£€]?\s*\d{1,6}[.,]\d{2}/, "").trim(), amount });
    }
...
server.tool(
  "deduction_finder",
  "Find applicable tax deductions based on profession or business type",
  {
  ...
  async ({ profession, annual_revenue, work_from_home }) => {
    ...
    const professionDeductions: { name: string; notes: string }[] = [];

    if (/design|creative|artist|photographer|videograph/.test(prof)) {
      professionDeductions.push(
        { name: "Adobe Creative Cloud / Design Software", notes: "100% deductible subscription" },
        { name: "Camera, Lighting, Studio Equipment", notes: "Section 179 or depreciation" },
        { name: "Stock Assets & Fonts", notes: "Purchased assets for client work" },
        { name: "Portfolio Website & Hosting", notes: "Domain, hosting, website builder" },
      );
    }
    if (/software|developer|engineer|tech|programmer/.test(prof)) {
      professionDeductions.push(
        { name: "Computer Hardware & Monitors", notes: "Section 179 expensing up to $1.16M" },
        { name: "SaaS Tools & Developer Subscriptions", notes: "GitHub, AWS, JetBrains, etc." },
        { name: "Cloud Services & Hosting", notes: "Production infrastructure costs" },
        { name: "Technical Books & Online Courses", notes: "Udemy, Coursera, O'Reilly, etc." },
      );
    }
    if (/real estate|realtor|broker/.test(prof)) {
      professionDeductions.push(
        { name: "MLS & Board Dues", notes: "National, state, and local association fees" },
        { name: "Vehicle Use for Showings", notes: "Mileage at $0.67/mile (2024 rate)" },
        { name: "Marketing & Advertising", notes: "Zillow, Realtor.com, yard signs, print" },
        { name: "E&O Insurance", notes: "Errors & omissions professional liability" },
      );
    }
    if (/consultant|advisor|coach|trainer/.test(prof)) {
      professionDeductions.push(
        { name: "Client Entertainment (50%)", notes: "Business meals are 50% deductible" },
        { name: "Travel to Client Sites", notes: "Flights, hotels, car rentals" },
        { name: "Proposal & Contract Tools", notes: "DocuSign, Proposify, Notion" },
        { name: "CRM & Sales Tools", notes: "HubSpot, Pipedrive, etc." },
      );
    }
    if (/writer|content|copywriter|journalist|blogger/.test(prof)) {
      professionDeductions.push(
        { name: "Research Materials & Subscriptions", notes: "Newspapers, journals, databases" },
        { name: "Writing Software", notes: "Scrivener, Grammarly, Hemingway Editor" },
        { name: "Domain & Blog Hosting", notes: "Personal publishing platform" },
      );
    }
...
server.tool(
  "mileage_calculator",
  "Convert business miles driven into an IRS-compliant tax deduction amount",
  {
  ...
  async ({ miles, year, trip_log }) => {
    const rates: Record<number, { business: number; medical: number; charity: number }> = {
...
server.tool(
  "tax_calendar",
  "Get upcoming tax deadlines and filing calendar based on entity type",
  {
  ...
  async ({ entity_type, state, fiscal_year_end }) => {
    ...
    const deadlines: Record<string, { date: string; form: string; description: string }[]> = {
      sole_proprietor: [
